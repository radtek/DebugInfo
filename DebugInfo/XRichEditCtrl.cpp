...
CXRichEditCtrl::CXRichEditCtrl()
{
	...
	_tcscpy(m_cfText.szFaceName,	_T("宋体"));
	...
	_tcscpy(m_cfGray.szFaceName, _T("宋体"));
	...
	_tcscpy(m_cfNumber.szFaceName, _T("宋体"));
}
...
void CXRichEditCtrl::SetDefaultFont()
{
	...
	FontTmp.CreateFont(12,0,0,0,650,FALSE,FALSE,0,
						...
						DEFAULT_QUALITY,VARIABLE_PITCH&FF_SWISS,_T("宋体"));
	SetFont(&FontTmp);
...
BOOL CXRichEditCtrl::MakeAscHex(CString str, CString &strNew, BOOL bToAsc)
{
	...
	{		
		...
		for(int i=0;i<nRead;i+=3)	
			scanf(str.Mid(i,2),_T("%02X"),&btTmp[i/3]);
		strNew=btTmp;
	}
	else
	{
		...
		{
			strTmp.Format(_T("%2X "),(BYTE)str.GetAt(i));
			strNew+=strTmp;
		}
		strTmp.Format(_T("%2X"),(BYTE)str.GetAt(nRead));
		strNew+=strTmp;	
		strTmp.Empty();
	}
	return 1;
	...
	for(i=0;i<nStrlength;i+=3)
	{
		...
		if((i+3)==nStrlength)
		{
			...
			if(isascii(lastchar)==0)	
				strError.Format(_T("最后多出一个非ASCII字符，其16进制为：  %02X H"),(BYTE)lastchar);
			else	strError.Format(_T("最后多出一个字符：  \' %c \'"),lastchar);
			nErrorPos=i+3;
			return 0;
		}
		TCHAR chr=' ';
		...
		{
			if(isascii(chr)==0)
				 strError.Format(_T("第 %d 个字节后为非ASCII字符，其16进制为：  %02X H。必须改为  空格"),i/3+1,(BYTE)chr);
			else strError.Format(_T("第 %d 个字节后的字符  \' %c \'  必须改为  空格"),i/3+1,chr);
			nErrorPos=i+3;
			return 0;
		}
		BOOL bHex=FALSE;
		for(int j=0;j<2;j++)
		{
			...
			if(chr==' ')	
			{
				if(j==0)	strError.Format(_T("第 %d 个字节的高位不能为  空格，必须是16进制数"),i/3+1);
				else		strError.Format(_T("第 %d 个字节的低位不能为  空格，必须是16进制数"),i/3+1);
				nErrorPos=i+j+1;
				return 0;
			}
			...
			if(!bHex)
			{
				...
				{
					if(isascii(chr)==0)
						 strError.Format(_T("第 %d 个字节的高位为非ASCII字符，其16进制为：  %02X H。必须是16进制数"),i/3+1,(BYTE)chr);
					else strError.Format(_T("第 %d 个字节的高位不能为  \' %c \'，必须是16进制数"),i/3+1,chr);
				}
				else	
				{
					if(isascii(chr)==0)
						 strError.Format(_T("第 %d 个字节的低位为非ASCII字符，其16进制为：  %2X H。必须是16进制数"),i/3+1,(BYTE)chr);
					else strError.Format(_T("第 %d 个字节的低位不能为  \' %c \'，必须是16进制数"),i/3+1,chr);
				}
				nErrorPos=i+j+1;
				return 0;
			}
...
BOOL CXRichEditCtrl::SaveFile(LPCTSTR str)
{
	...
	SetModify(FALSE);
	AfxGetApp()->WriteProfileString(_T("Settings") ,_T("XREPath"), str);
	fld.Close();
	return 1;
}
static DWORD CALLBACK 
...
{
	if((btData==NULL)||(nLength<1))	return _T("");
...
int CXRichEditCtrl::StrToHex(CString str,BYTE* btData)
{
	...
	int i;
	for(i=0;i<nLength;i+=3)	_stscanf(str.Mid(i,2),_T("%02X"),&btData[i/3]);
	return (i+1)/3;
}
...
int CXRichEditCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	{
		AfxMessageBox(_T("RICHED20.DLL 未找到"),
	...
	case WM_KEYDOWN:
		if(IsCASDown(4))//ctrl+alt
		{
			...
			//	SetSel(GetLineStart(),GetLineEnd());
				if(!GetLineStr().IsEmpty())	ReplaceStr(_T(""),5);
				SetSel(GetLineStart(),GetLineStart());
...
{
	CString str;
	str=AfxGetApp()->GetProfileString(_T("Settings"),_T("XREPath"));
	...
	str=str.Left(position+1)+AfxGetApp()->m_pszExeName+_T(".pcf");
	return str;
}
